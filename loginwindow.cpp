...
void LoginWindow::do_login(const QString& username, const QString& passwd)
{
    ...
    if (q.next()) {
        ...
        QProgressDialog* progress = new QProgressDialog("Signing in...", "Abort", 0, 0, this);
        progress->setValue(0);
        progress->setMinimumDuration(1000);

        // connect(qnam, SIGNAL(finished(QNetworkReply*)),this, SLOT(on_login_finished(QNetworkReply*)));
        connect(qnam, &QNetworkAccessManager::finished, qnam, [this, progress](QNetworkReply* reply){
            progress->cancel();
            this->on_login_finished(reply);
        });
